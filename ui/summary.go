...
// NewSummary returns a new Summary struct. It returns an error if the
// passed stats fields cannot be used as a trend column.
func NewSummary(sts []string) (*Summary, error) {
	tc := []string{"avg", "min", "med", "max", "p(90)", "p(95)"}
	if len(sts) > 0 {
		tc = sts
	}

	resolvers := map[string]func(s *stats.TrendSink) interface{}{
	...
	s := Summary{trendColumns: tc, trendValueResolvers: resolvers}
...
func (s *Summary) Validate() error {
	for _, stat := range s.trendColumns {
		if stat == "" {
